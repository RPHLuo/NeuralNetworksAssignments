...
class FeedForwardNetwork:
    ...
    def __init__(self, shape):
        ...
        self.w_h1 = self.init_weights([2, size_h1])
        self.w_o = self.init_weights([size_h1, 1])
        self.py_x = model(X, w_h1, w_o)

        self.learningRate = 0.02
        self.tolerance = 0.02
        self.cost = tf.losses.mean_squared_error(py_x, Y)
        self.train_op = tf.train.MomentumOptimizer(learningRate,0.9).minimize(cost)
